...
vector<T, Allocator>::vector(size_type count, const allocator_type& alloc) : m_begin(nullptr), m_end(nullptr), m_cap(nullptr), m_allocator(alloc)
{
    if (count > 0)
    {
        m_begin = allocator_traits::allocate(m_allocator, count);
        m_end = m_begin;
        m_cap = m_begin + count;
        ...
        try
        {
            ...
            {
                allocator_traits::construct(m_allocator, m_end);  // 使用allocator_traits构造
                ++m_end;
            }
        }
        catch (...)
        {
            ...
            allocator_traits::deallocate(m_allocator, m_begin, count);
            m_begin = nullptr;
            m_end = nullptr;
            m_cap = nullptr;
            throw;
...
vector<T, Allocator>::vector(size_type count, const T& value, const allocator_type& alloc) : m_begin(nullptr), m_end(nullptr), m_cap(nullptr), m_allocator(alloc)
{
    if (count > 0)
    {
        ...
        try
        {
            for (size_type i = 0; i < count; ++i)
            {
                allocator_traits::construct(m_allocator, m_end, value);  // 使用allocator_traits拷贝构造
                ++m_end;
            }
        }
        catch (...)
    ...
    if (first != last)
    {
        ...
        if (count > 0)
        {
            ...
            try
            {
                for (InputIt it = first; it != last; ++it)
                {
                    allocator_traits::construct(m_allocator, m_end, *it);  // 使用allocator_traits拷贝构造
                    ++m_end;
                }
            }
            catch (...)
            {
                // 销毁已构造的元素
                for (iterator iter = m_begin; iter != m_end; ++iter)
                {
                    allocator_traits::destroy(m_allocator, iter);
                }
                allocator_traits::deallocate(m_allocator, m_begin, count);
                m_begin = nullptr;
                m_end = nullptr;
...
vector<T, Allocator>::~vector()
{
    ...
    if (m_begin)
    {
        allocator_traits::deallocate(m_allocator, m_begin, m_cap - m_begin);
    }
}
...
vector<T, Allocator>::vector(const vector& other) : m_begin(nullptr), m_end(nullptr), m_cap(nullptr), m_allocator(other.m_allocator)
{
    ...
    if (other_size > 0)
    {
        ...
        // 先分配内存
        m_begin = allocator_traits::allocate(m_allocator, other_capacity);
        m_end = m_begin;
        ...
        try
        {
            ...
            {
                allocator_traits::construct(m_allocator, m_end, *src_it);  // 使用allocator_traits拷贝构造
            }
        }
        catch (...)
        {
            ...
            allocator_traits::deallocate(m_allocator, m_begin, other_capacity);
            m_begin = nullptr;
            m_end = nullptr;
            m_cap = nullptr;
            throw;  // 重新抛出异常
    ...
    else
    {
        ...
        if (other_size > 0)
        {
            ...
            try
            {
                for (iterator src_it = other.m_begin; src_it != other.m_end; ++src_it, ++m_end)
                {
                    allocator_traits::construct(m_allocator, m_end, std::move(*src_it));
                }
            }
            catch (...)
            {
...
void vector<T, Allocator>::push_back(const T& value)
{
    ...
    // 使用allocator_traits在指定位置构造元素
    allocator_traits::construct(m_allocator, m_end, value);
    ++m_end;
}
...
void vector<T, Allocator>::push_back(T&& value)
{
    ...
    // 使用allocator_traits移动构造在指定位置构造元素
    allocator_traits::construct(m_allocator, m_end, std::move(value));
    ++m_end;
}
...
void vector<T, Allocator>::pop_back()
{
    ...
    {
        --m_end;
        allocator_traits::destroy(m_allocator, m_end);  // 使用allocator_traits销毁元素
    }
}
...
void vector<T, Allocator>::emplace_back(Args&&... args)
{
    ...
    // 使用allocator_traits在指定位置直接构造元素
    allocator_traits::construct(m_allocator, m_end, std::forward<Args>(args)...);
    ++m_end;
}
...
typename vector<T, Allocator>::iterator vector<T, Allocator>::insert(iterator pos, const T& value)
{
    ...
    {
        allocator_traits::construct(m_allocator, it, std::move(*(it - 1)));
        allocator_traits::destroy(m_allocator, it - 1);
    }

    // 在指定位置构造新元素
    allocator_traits::construct(m_allocator, m_begin + index, value);
    ++m_end;
...
typename vector<T, Allocator>::iterator vector<T, Allocator>::insert(iterator pos, T&& value)
{
    ...
    // 在指定位置移动构造新元素
    allocator_traits::construct(m_allocator, m_begin + index, std::move(value));
    ++m_end;
...
typename vector<T, Allocator>::iterator vector<T, Allocator>::erase(iterator pos)
{
    ...
    // 销毁要删除的元素
    allocator_traits::destroy(m_allocator, pos);
...
typename vector<T, Allocator>::iterator vector<T, Allocator>::erase(iterator first, iterator last)
{
    ...
    {
        allocator_traits::construct(m_allocator, it, std::move(*last));
...
void vector<T, Allocator>::clear()
{
    for (iterator it = m_begin; it != m_end; ++it)
    {
        allocator_traits::destroy(m_allocator, it);  // 使用allocator_traits销毁每个元素
    }
    m_end = m_begin;
}
...
void vector<T, Allocator>::reserve(size_type newCapacity)
{
    ...
    if (newCapacity > current_capacity)
    {
        ...
        // 分配新内存
        T* newElements = allocator_traits::allocate(m_allocator, newCapacity);
        ...
        try
        {
            ...
            {
                allocator_traits::construct(m_allocator, &newElements[i], std::move(*it));
            }
        }
        catch (...)
        {
            // 如果移动失败，释放新分配的内存
            allocator_traits::deallocate(m_allocator, newElements, newCapacity);
        ...
        if (m_begin)
        {
            allocator_traits::deallocate(m_allocator, m_begin, current_capacity);
        }
...
void vector<T, Allocator>::shrink_to_fit()
{
    ...
    if (capacity() > current_size)
    {
        ...
        // 分配新内存
        T* newElements = allocator_traits::allocate(m_allocator, current_size);
        ...
        {
            // 如果移动失败，释放新分配的内存
            allocator_traits::deallocate(m_allocator, newElements, current_size);
            throw; // 重新抛出异常，保持原有对象状态不变
        }
        ...
        allocator_traits::deallocate(m_allocator, m_begin, old_capacity);
...
void vector<T, Allocator>::resize(size_type count, const T& value)
{
    ...
    else if (count > current_size)
    {
        ...
        {
            allocator_traits::construct(m_allocator, m_end, value); // 使用allocator_traits拷贝构造新元素
            ++m_end;
        }
    }
